...
const totalContributes = GAMES_JSON.reduce( (acc, game) => {
    return acc + game.backers;
}, 0);

// set the inner HTML using a template literal and toLocaleString to get a number with commas
contributionsCard.innerHTML = `${totalContributes.toLocaleString( 'en-US')}`;
...
let gamesUnfunded = GAMES_JSON.reduce( (acc, game) => {
    return game.pledged < game.goal ? acc + 1 : acc;
}, 0);

// create a string that explains the number of unfunded games using the ternary operator
const displayStr = `A total of $${totalAmountRaised.toLocaleString('en-US')} has been raised for ${totalGames} games. Currently, ${gamesUnfunded} ${gamesUnfunded === 1 ? "game remain" : "games remain"} unfunded. We need your help to fund ${gamesUnfunded === 1 ? "this amazing game" : "these amazing games"}!`;
